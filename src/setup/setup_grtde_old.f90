...
subroutine setpart(id,npart,npartoftype,xyzh,massoftype,vxyzu,polyk,gamma,hfact,time,fileprefix)
 ...
 use externalforces, only:accradius1,accradius1_hard,iext_corotate,omega_corotate
...
!
 ...
 !
 nptmass_in = 0
 if (nstar > 1 .and. use_set_binary) then
 ...
 if (nstar == 1 .or. use_set_binary) then  
 ...
 print*,star%iprofile,"star%iprofile"
 if (use_MC_code) then 
    do i=1,nstar
       if (star(i)%iprofile > 0) then
           if (i == 1) then
           ...
           call shift_star(npart,xyzh,vxyzu,x0=xyzmh_ptmass(1:3,i)+xyzstar,&
            ...
            print*,"adding nptmass",star(1)%hacc,"hacc",ierr,"ierr" ,xyzmh_ptmass(:,nptmass)," xyzmh_ptmass(:,nptmass) before",star(1)%mstar,"star(1)%mstar"
            nptmass = nptmass + 1
            if (i == 1) then
              xyzstar = (/x_pos1,y_pos1,z_pos1/)
              vxyzstar = (/vx_pos1,vy_pos1,vz_pos1/)
            ...
            xyzmh_ptmass(4,nptmass) = star(i)%mstar
            xyzmh_ptmass(5,nptmass) = star(i)%hacc
            xyzmh_ptmass(1:3,nptmass) = xyzstar
            vxyz_ptmass(1:3,nptmass) = vxyzstar
            print*, xyzmh_ptmass(:,nptmass)," xyzmh_ptmass(:,nptmass) after"
         endif
      enddo
 ...
 elseif (nstar > 1 .and. use_set_binary) then 
    do i=1,nstar
